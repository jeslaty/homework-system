...
date_str = current_date.strftime("%Y-%m-%d")  # 轉換成文字格式如 2026-09-04
    ...
    for index, row in df.iterrows():
        ...
        if extra_items:
            for item in extra_items:
                ...
                item_opts = ["已繳 ✅", "未繳 ❌"]
                i_idx = (
                    item_opts.index(current_item_status)
                    if current_item_status in item_opts
                    else 1
                )
...
with col2:
    ...
    if not no_sign_df.empty:
        ...
        st.text_area(
            "可複製傳至家長群組：", value=sign_text, height=120, key=f"copy_sign_{date_str}"
        )
    else:
